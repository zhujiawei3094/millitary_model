...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.x770 = self.create_parameter(dtype='float32', shape=(), default_initializer=paddle.nn.initializer.Constant(value=8.0))
        self.conv2d58 = paddle.nn.Conv2D(out_channels=18, kernel_size=(1, 1), in_channels=256)
        self.sigmoid1 = paddle.nn.Sigmoid()
        self.x799 = self.create_parameter(dtype='float32', shape=(), default_initializer=paddle.nn.initializer.Constant(value=16.0))
        self.conv2d59 = paddle.nn.Conv2D(out_channels=18, kernel_size=(1, 1), in_channels=512)
        self.sigmoid2 = paddle.nn.Sigmoid()
        self.x828 = self.create_parameter(dtype='float32', shape=(), default_initializer=paddle.nn.initializer.Constant(value=32.0))
...
def main(x0):
    ...
    params = paddle.load(r'/kaggle/working/millitary_model/v5/tank_11_1_120eps_best_paddle_model/model.pdparams')
